...
namespace DotMinecraft
{
	...
	public sealed class WorldManager : IMinecraftWorldView
	{
		...
		public static byte[] SerializeChunkPacket(int chunkX, int chunkZ, IMinecraftWorldView world)
		{
			using var ms = new MemoryStream();
			var encoder = new MinecraftProtocolEncoder(ms);

			encoder.WriteUnmanagedBigEndian(chunkX);
			encoder.WriteUnmanagedBigEndian(chunkZ);
			encoder.WriteBool(true); // Ground-up continuous

			int sectionMask = 0;
			var sectionStreams = new List<MemoryStream>();

			// Collect non-empty sections
			for (int sectionY = 0; sectionY < 16; sectionY++)
			{
				ushort[] blockIds = new ushort[4096];
				bool nonEmpty = false;

				int i = 0;
				for (int y = 0; y < 16; y++)
				{
					int yAbs = sectionY * 16 + y;
					for (int z = 0; z < 16; z++)
					{
						for (int x = 0; x < 16; x++, i++)
						{
							var id = world.ReadBlock(new PreciseCoordinate(x, yAbs, z), 0).id;
							blockIds[i] = id;
							if (id != 0) nonEmpty = true;
						}
					}
				}
				...
				var sectionEncoder = new MinecraftProtocolEncoder(sectionStream);

				var palette = new List<ushort>();
				var paletteMap = new Dictionary<ushort, int>();
				var indices = new int[4096];

				for (int j = 0; j < 4096; j++)
				{
					ushort id = blockIds[j];
					if (!paletteMap.TryGetValue(id, out int index))
					{
						index = palette.Count;
						palette.Add(id);
						paletteMap[id] = index;
				...
				sectionEncoder.WriteByte((byte)sectionY);
				sectionEncoder.WriteVarint(bitsPerBlock);
				sectionEncoder.WriteVarint(palette.Count);
				foreach (var id in palette)
					sectionEncoder.WriteVarint(id);

				// Write packed data array
				int entriesPerLong = 64 / bitsPerBlock;
				int longCount = (int)Math.Ceiling(4096 / (double)entriesPerLong);
				sectionEncoder.WriteVarint(longCount);

				ulong[] packed = new ulong[longCount];
				for (int j = 0; j < 4096; j++)
				{
					int idx = j / entriesPerLong;
					int offset = (j % entriesPerLong) * bitsPerBlock;
					packed[idx] |= ((ulong)indices[j] << offset);
				}

				foreach (var l in packed)
					sectionEncoder.WriteUnmanagedBigEndian<ulong>(l);

				// Zero block light & sky light
				sectionEncoder.WriteDirect(new byte[2048]); // block light
				sectionEncoder.WriteDirect(new byte[2048]); // sky light

				sectionStreams.Add(sectionStream);
			}

			encoder.WriteVarint(sectionMask);

			// Buffer section data to measure total size
			var dataBuffer = new MemoryStream();
			foreach (var s in sectionStreams)
				s.WriteTo(dataBuffer);

			// Heightmaps (empty compound tag): 0x0A 0x00 0x00 0x00
			dataBuffer.WriteByte(0x0A); // TAG_Compound
			dataBuffer.WriteByte(0x00); // name length high
			dataBuffer.WriteByte(0x00); // name length low
			dataBuffer.WriteByte(0x00); // TAG_End

			// Biomes not sent; client fills with default
			dataBuffer.WriteByte(0x00); // no biomes

			byte[] rawSectionData = dataBuffer.ToArray();
			encoder.WriteVarint(rawSectionData.Length);
			encoder.WriteDirect(rawSectionData);
